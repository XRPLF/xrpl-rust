...
/// (matching the xrpld JSON API format) rather than a JSON array.
pub(crate) mod opt_lgr_obj_flags {
    ...
    mod tests {
        use super::*;
        use crate::models::transactions::mptoken_issuance_create::MPTokenIssuanceCreateFlag;
        use alloc::vec;
        use serde::{Deserialize, Serialize};

        #[derive(Debug, PartialEq, Serialize, Deserialize)]
        struct OptionalFlagsWrapper {
            #[serde(with = "super")]
            flags: Option<FlagCollection<MPTokenIssuanceCreateFlag>>,
        }

        #[test]
        fn test_serialize_some_non_empty_flags_as_integer() {
            let wrapper = OptionalFlagsWrapper {
                flags: Some(
                    vec![
            ...
            assert_eq!(serde_json::to_string(&wrapper).unwrap(), r#"{"flags":34}"#);
        }

        #[test]
        fn test_serialize_empty_and_none_flags() {
            let empty = OptionalFlagsWrapper {
                flags: Some(FlagCollection::default()),
            };
            assert_eq!(serde_json::to_string(&empty).unwrap(), r#"{"flags":0}"#);

            let none = OptionalFlagsWrapper { flags: None };
            assert_eq!(serde_json::to_string(&none).unwrap(), r#"{"flags":null}"#);
        }

        #[test]
        fn test_deserialize_optional_flags() {
            let wrapper: OptionalFlagsWrapper = serde_json::from_str(r#"{"flags":34}"#).unwrap();
            let flags = wrapper.flags.expect("flags should deserialize");
