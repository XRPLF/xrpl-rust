...
const TF_MPT_LOCK_FLAG: u32 = 0x00000001;
const TF_MPT_UNLOCK_FLAG: u32 = 0x00000002;
...
impl TryFrom<u32> for MPTokenIssuanceSetFlag {
    ...
    fn try_from(value: u32) -> Result<Self, Self::Error> {
        match value {
            value if value == MPTokenIssuanceSetFlag::TfMPTLock as u32 => {
                Ok(MPTokenIssuanceSetFlag::TfMPTLock)
            }
            value if value == MPTokenIssuanceSetFlag::TfMPTUnlock as u32 => {
                Ok(MPTokenIssuanceSetFlag::TfMPTUnlock)
            }
            _ => Err(XRPLModelException::InvalidValue {
                field: "flags".into(),
                expected: "a known MPTokenIssuanceSet flag bit".into(),
...
impl MPTokenIssuanceSetFlag {
    pub fn from_bits(bits: u32) -> Vec<Self> {
        ...
        if bits & MPTokenIssuanceSetFlag::TfMPTLock as u32 != 0 {
            flags.push(MPTokenIssuanceSetFlag::TfMPTLock);
        }
        if bits & MPTokenIssuanceSetFlag::TfMPTUnlock as u32 != 0 {
...
impl<'a> MPTokenIssuanceSet<'a> {
    ...
    fn _get_flag_error(&self) -> XRPLModelResult<()> {
        ...
        // xrpld preflight rejects only when both flags are set simultaneously.
...
mod tests {
    ...
    use crate::models::transactions::test_fixtures::{
        GENESIS_ACCOUNT, INVALID_MPT_ISSUANCE_ID_NON_HEX, INVALID_MPT_ISSUANCE_ID_SHORT,
        ISSUER_ACCOUNT, MPT_ISSUANCE_ID,
    };
    ...
    fn test_no_flag_is_valid() {
        // No-flag submissions are valid per xrpld (e.g. DomainID-only changes).
        // xrpld only rejects when both TfMPTLock and TfMPTUnlock are set simultaneously.
        let txn = MPTokenIssuanceSet {
            common_fields: CommonFields {
                account: ISSUER_ACCOUNT.into(),
                transaction_type: TransactionType::MPTokenIssuanceSet,
    ...
    fn test_invalid_mptoken_issuance_id_non_hex() {
        let txn = MPTokenIssuanceSet {
            ...
            mptoken_issuance_id: INVALID_MPT_ISSUANCE_ID_NON_HEX.into(),
            ..Default::default()
        };
    ...
    fn test_flag_try_from_u32() {
        assert_eq!(
            MPTokenIssuanceSetFlag::try_from(MPTokenIssuanceSetFlag::TfMPTLock as u32),
            Ok(MPTokenIssuanceSetFlag::TfMPTLock)
        );
        assert_eq!(
            MPTokenIssuanceSetFlag::try_from(MPTokenIssuanceSetFlag::TfMPTUnlock as u32),
            Ok(MPTokenIssuanceSetFlag::TfMPTUnlock)
        );
        assert!(MPTokenIssuanceSetFlag::try_from(
            (MPTokenIssuanceSetFlag::TfMPTUnlock as u32) << 1
        )
        .is_err());
    }
    ...
    fn test_flag_from_bits() {
        let flags = MPTokenIssuanceSetFlag::from_bits(
            MPTokenIssuanceSetFlag::TfMPTLock as u32 | MPTokenIssuanceSetFlag::TfMPTUnlock as u32,
        );
