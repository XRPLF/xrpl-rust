...
- `definitions.json` regenerated from `server_definitions` on rippled 3.4.0-rc1. Adds the `LendingProtocolV1_1` fields (`VaultKind`, `SubscriptionDate`, `RedemptionDate`, `LEVersion`) and drops the Hooks fields, which rippled no longer supports. No existing field's code, type or signing flag changed.
