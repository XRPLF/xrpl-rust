...
- Support for the [`LendingProtocol` amendment (XLS-66)](https://github.com/XRPLF/XRPL-Standards/pull/192): the `LoanBrokerSet`, `LoanBrokerDelete`, `LoanBrokerCoverDeposit`, `LoanBrokerCoverWithdraw`, `LoanBrokerCoverClawback`, `LoanSet`, `LoanPay`, `LoanManage` and `LoanDelete` transactions, the `Loan` and `LoanBroker` ledger objects, and `signing::sign_loan_set_by_counterparty` ([#162](https://github.com/XRPLF/xrpl-rust/pull/162)).
- Support for the `LendingProtocolV1_1` amendment ([XLS-0058](https://github.com/XRPLF/XRPL-Standards/pull/582), [XLS-0066 V1_1](https://github.com/XRPLF/XRPL-Standards/pull/587)): close-ended vaults (`VaultKind`, `SubscriptionDate` and `RedemptionDate` on `VaultCreate` and on the `Vault` ledger object), `VaultDelete.MemoData`, and `CredentialIDs` on `VaultWithdraw` and `LoanBrokerCoverWithdraw`.
- Support for the `fixCleanup3_4_0` counterparty signer prefixes: `encode_for_signing_counterparty` (`"CPT\0"`) and `encode_for_multisigning_counterparty` (`"CPM\0"`), used by `signing::sign_loan_set_by_counterparty`. A counterparty signature produced by an earlier version does not validate on a `fixCleanup3_4_0` server.
- `signing::combine_loan_set_counterparty_signers`, which merges counterparty signatures produced independently — on separate machines, say — into a single `LoanSet`.
- `signing::CounterpartySigningMode`, the third argument to `sign_loan_set_by_counterparty`: `Single`, `Multisign`, or `MultisignAs(account)` for a wallet signing with that account's RegularKey (parity with xrpl.js's `opts.multisign?: boolean | string`).
- `autofill` now covers a `LoanSet`'s counterparty signatures in the `Fee`, reading the counterparty's signer list (and resolving the counterparty from `LoanBrokerID` when the broker initiates). `Fee` is a signing field and so cannot be raised once the first party has signed, which previously made this the caller's problem.
- `ledger_entry` selectors for `Loan` and `LoanBroker` (`LoanIdentifier` and `LoanBrokerIdentifier`).
...
- `definitions.json` regenerated from `server_definitions` on rippled 3.4.0-rc1: adds the `LendingProtocolV1_1` fields (`VaultKind`, `SubscriptionDate`, `RedemptionDate`, `LEVersion`), the smart-contract and confidential-MPT key-epoch fields, six new transaction results, and the `SponsorshipSet` / `EscrowCreate` format updates; drops the Hooks fields, which rippled no longer supports. No existing field's code, type or signing flag changed.
